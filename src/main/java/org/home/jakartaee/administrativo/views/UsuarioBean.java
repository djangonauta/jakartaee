...
public class UsuarioBean implements Serializable {
    ...
    private boolean isDadosEstaoCorretos() {
        ...
        boolean senhasCombinam = isSenhasCombinam();
        boolean senhasNaoCombinam = !senhasCombinam;

        if (loginExiste) {
            facesContext.addMessage("form:login",
                    new FacesMessage("Já existe um usuário com esse login"));
        }

        if (emailExiste) {
            facesContext.addMessage("form:email",
                    new FacesMessage("Já existe um usuário com esse email"));
        }

        if (senhasNaoCombinam) {
            facesContext.addMessage("form:confirmarSenha",
                    new FacesMessage("Senhas não combinam"));
        }

        return loginNaoExiste && emailNaoExiste && senhasCombinam;
    }

    private boolean isSenhasCombinam() {
        return StringUtils.equals(senha, confirmarSenha);
    }
